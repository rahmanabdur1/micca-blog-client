...
const PaginationBlog = ({ currentPage }) => {
  ...
  useEffect(() => {
    async function fetchBlogsData() {
      try {
        const response = await fetch(`http://localhost:5000/blogs?page=${currentPage}&limit=${itemsPerPage}`);
        if (!response.ok) {
  ...
  return (
    <div className={`pagination-blog-container ${theme ? 'dark' : ''}`}>
      {currentPage === 1 ? <h3 className='latest-blog'>LATEST BLOGS</h3> :
        <div className='all-blogs'>
          <h2>All Blogs</h2>
          <p>Commodo ea nisi enim aute veniam reprehenderit cillum laborum aliquip sit eiusmod occaecat laborum fugiat mollit.</p>
        </div>
      }
